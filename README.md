...
      - [data.htm](#datahtm)  
      - [index.htm](#indexhtm)  
  2.3 [Загальний список проектів](#Загальний-список-проектів)  
...
Нижче наведена спрощена структура сайту, де прокоментовані директорії і файли в які можна вносити правки. Непрокоментовані тут директорії а також відсутні тут файли краще не займати, щоби сайт не зламався :).

```
cgi-bin/
    projects.py   - список проектів
public_html/
    _inc/ 
...
        _all.htm  - список (усіх) навичок для головної сторінки
        *.htm     - файли навичок
    .htaccess
    index.htm
...
В кожній директорії проекту має бути 4 файли. Краще не створювати файли з нуля, а скопіювати їх з проекту **_blank** і відредагувати.
...
SKILL3 - скіл 3
```
DESCR - може бути порожнім: `''`, як і SKILL2, SKILL3.
...
При необхідності надати більше інформації про проект (додаткові зображення, текст, посилання) це можна зробити у файлі index.htm, що знаходиться в директорії проекту, між мітками  
`<!-- ADDITIONAL CODE HERE -->` і `<!--#include virtual=...`.

За допомогою текстового редактора додаємо необхідний HTML-код. Наприклад:
...
## 2.3 Загальний список проектів

Цей файл _**cgi-bin/projects.py**_ повідомляє скриптам про наявність проекта

Звичайно, за допомогою скриптів можна було просканувати директорію _**projects**_

Але у цього файла є важлива функція: він вказує на порядок слідування проектів. Від цього порядку залежать і посилання на наступний і попередній проекти на сторінці поточного проекту.
Найновіші проекти додаються на початок списку.
...
додаємо новий проект (не забуваємо про лапки і кому):
...
...
```
Не забудьте додати скіл на головну сторінку! Для цього редагуємо файл _**skill/_all.htm**_ 
```
<ul id="filterNav" class="clearfix">
	<li class="allBtn"><a href="#" data-filter="*" class="selected">All</a></li>
  ...
 	<li class="mynewskilBtn"><a href="#" data-filter=".mynewskil">mynewskil</a></li>          # 3(!) входження рядка 'mynewskil'
</ul>
...
Оновлення файлів на сервері здійснюється через налаштоване FTP-з’єднання.

Якщо ми локально додали нові діректорії і файли - ми повинні їх скопіювати на сервер.

Якщо ми локально змінили вже існуючі файли - нам треба замінити відповідні файли на сервері.

Проекти у _**/projects/**_, скіли у _**/skill/**_, about у _**/about/**_......
